...
Configure the learner project's own AppHost store once. From `interview-coach-lab`, replace the first three values with the names you kept from Chapter 0. Use the Azure **deployment name**, usually `chat`, rather than the model name. The commands read the active subscription and the account's location automatically:

<Tabs syncKey="shell">
<TabItem label="Bash">

```bash
foundryName="YOUR_ACCOUNT_NAME"
resourceGroup="YOUR_RESOURCE_GROUP"
deploymentName="YOUR_DEPLOYMENT_NAME"
subscriptionId=$(az account show --query id --output tsv)
tenantId=$(az account show --query tenantId --output tsv)
location=$(az cognitiveservices account show --name "$foundryName" --resource-group "$resourceGroup" --subscription "$subscriptionId" --query location --output tsv)
...
$tenantId = az account show --query tenantId --output tsv
$location = az cognitiveservices account show --name $foundryName --resource-group $resourceGroup --subscription $subscriptionId --query location --output tsv
if ($LASTEXITCODE -ne 0) { throw "Check the account name, resource group, and active subscription." }
